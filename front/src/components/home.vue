        ...
        <table class="table center">
          <thead>
            <tr>
              <th>Point</th>
              <th
                v-for="campaign in measurepointChoiceDict"
                :key="campaign.id"
              >{{campaign.reference}}</th>
            </tr>
          </thead>
    ...
    <div v-if="result" class="selection-menu">
      ...
      >
        ...
        <div class="dropdown-menu">
          <div class="dropdown-content">
            ...
            >Streets</a>
            <a class="dropdown-item" @click="changeBackground('light-v10');mapChosen='Light';">Light</a>
      ...
      <div v-show="LbRegionHyChosen" class="selection-item console">
        ...
        <div class="selection-item">
          ...
          <a @click="changeLayerColor">
            <v-swatches swatches="text-advanced" class="selection-item" v-model="colorChosen" />
          </a>
        </div>
        <div v-if="sliderShown" class="selection-item">Opacité: {{sliderValue}}%</div>
...
export default {
  ...
  methods: {
        ...
        if (!alreadyShown) {
          ...
          campaignChosen.place.forEach(placeChecked => {
            placeChecked.measurepoint.forEach(measurepointChecked => {
              if (
                t0IdList.find(t0Id => t0Id == measurepointChecked.code_t0_id) ==
                undefined
              ) {
    ...
    addPoint(number, longitude, latitude, project, isMeasurepoint, reference) {
      ...
      marker.on("dragstart", () => {
        if (this.pointArrowIdList.find(layer => layer == `point-arrow-${id}`)) {
      ...
      element.textContent = this.showNumber ? number : "";
    },
      ...
      if (!this.biotestChosen) {
        this.currentPoints.forEach(point => {
          ...
          if (!point.shown) {
            ...
            DOMElement.textContent = this.showNumber ? point.number : "";
          }
        });
      }
      if (this.biotestChosen) {
            ...
            this.campaignChosenList.forEach((campaign, index) => {
              ...
              if (point.isMeasurepoint) {
                measurepointChecked = this.result.campaign
                  .find(resultCampaign => resultCampaign.reference == campaign)
                        ...
                        (this.measurepointChoiceDict
                          ...
                          measurepoint.reference ==
                            this.measurepointChoiceDict
                              ...
                              .measurepointShown.find(m => m.shown).reference))
                  );
                }
    ...
    async addStreamLayer() {
      if (!this.streamData) {
        const { data } = await axios.get(
          `${process.env.VUE_APP_API_URL}/layer/stream`
        );
        this.streamData = data;
      }
      if (!this.streamAdded) {
        this.map.addSource("Stream", {
          type: "geojson",
          data: {
            type: "FeatureCollection",
            features: this.streamData
          },
          generateId: true
        });
    ...
    removeStream() {
      this.map.removeLayer("streams");
      this.map.removeSource("Stream");
      ...
      if (this.lastColorChosen != this.colorChosen) {
        ...
        if (alreadyChanged) {
          ...
          this.changedColorFeature.splice(
            this.changedColorFeature.findIndex(color => color.sector == sector),
            1
          );
